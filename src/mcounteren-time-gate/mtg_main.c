// mtg_main.c: mcounteren time-counter gate check (backlog item 170).
//
// Mechanism under test: mcounteren gates each counter independently
// for S-mode (and U-mode). With mcounteren = 0x1 (CY set, TM clear)
// an S-mode rdtime must raise an illegal-instruction exception
// (scause = 2) while an S-mode rdcycle still reads and advances.
// This is the time-counter counterpart of src/scounteren-trap,
// which gated the cycle counter; the two experiments differ only
// in which counter bit is cleared. M-mode reads are never gated,
// so the experiment needs a real privilege drop to observe the
// gate.
//
...
//        exactly 0x7, so the later 0x1 write is proven to take
//        effect rather than be silently ignored.
//     2. Write mcounteren = 0x1 (CY only, TM clear) and require
//        the readback to be 0x1.
//     3. Install the M-mode mtvec handler (parks the hart; no
...
//     4. mret with MPP=01 into mtg_smode_test.
//   S-mode payload:
//     A. Execute rdtime at a labeled 4-byte site with
//        mcounteren.TM = 0. Expected: exactly one S-mode trap with
//        scause = 2, sepc exactly at the rdtime site, the
//        handler's +4 advance landing on the labeled resume
//        address, and the destination register still holding its
//        pre-fault sentinel (the rdtime never retired).
//     B. Execute rdcycle twice at a labeled site with
//        mcounteren.CY = 1. Expected: no new trap, a strictly
//        positive first sample, and a strictly increasing second
//        sample.
//   Verdict: RESULT: PASS only if all 11 checks held. On PASS the
...
extern void mtg_strap_entry(void);
extern void mtg_mtrap_entry(void);

// S-mode trap record, written by mtg_strap.S:
// [0] scause, [1] stval, [2] sepc at entry, [3] trap count,
// [4] sepc after the +4 advance, [5] interrupted t0.
volatile unsigned long mtg_st_regs[6];
...
// identical on every passing run. Absolute cycle/time sample
// values are NOT fed (they vary run to run); only the check
// outcomes and deterministic values go into the checksum.
...
void mtg_smode_test(void) {
    unsigned long site, after, v;
    unsigned long site2, after2, c0, c1;
    unsigned long scause, stval, sepc, sepc_after, traps;
    int ok;

    uart_puts("in S-mode; mcounteren time-gate experiment begins\n\n");

    // Phase A: mcounteren = 0x1 (TM clear), so this S-mode rdtime
    // must trap.
    v = SENTINEL;
    __asm__ volatile(
        ".option push\n\t"
    ...
    scause = mtg_st_regs[0];
    stval = mtg_st_regs[1];
    sepc = mtg_st_regs[2];
    traps = mtg_st_regs[3];
    sepc_after = mtg_st_regs[4];

    uart_puts("phase A: rdtime with mcounteren=0x1 (expect S-mode trap):\n");
    ...
    cks_feed(scause);
    ok = (sepc == site);
    check(ok, "phase A: sepc is not at the rdtime site");
    cks_feed(ok);
    ok = (sepc_after == after);
    ...
    // Phase B: with the CY bit still set, the S-mode rdcycle must
    ...
    traps = mtg_st_regs[3];
    uart_puts("\nphase B: rdcycle with mcounteren=0x1 (expect success):\n");
    uart_puts("  fault site (label 1) = ");
    uart_put_hex(site2);
    uart_puts("\n  sample 1 = ");
    uart_put_hex(c0);
    ...
    ok = (traps == 1);
    check(ok, "phase B: unexpected S-mode trap on rdcycle");
    cks_feed(traps);
    ok = (c0 > 0);
    check(ok, "phase B: first rdcycle sample is not strictly positive");
    cks_feed(ok);
    ok = (c1 > c0);
...
int main(void) {
    ...
    uart_init();
    uart_puts("\n========================================\n");
    uart_puts("mcounteren: S-mode rdtime gating trap\n");
    uart_puts("mcounteren=0x1: rdtime must trap, rdcycle must read\n");
    uart_puts("========================================\n\n");

    // 1. Boot mcounteren and the writability probe: write 0x7
    // (CY|TM|IR), read back; the readback must be exactly 0x7 so
    // the later 0x1 write is proven to take effect rather than be
    // silently ignored (a lone 0x1 write would be ambiguous if
    ...
    // 2. Gate: write mcounteren = 0x1 (CY set, TM and IR clear),
    // read back.
    write_mcounteren(0x1UL);
    rb = read_mcounteren();
    uart_puts("write: csrw mcounteren, 0x1; readback=");
    uart_put_hex(rb);
    uart_puts("\n");
    check(rb == 0x1UL, "mcounteren write of 0x1 did not read back as 0x1");
    cks_feed(rb);

    // 3. Trap vectors, delegation, PMP.
    __asm__ volatile("la t0, mtg_mtrap_entry\n\t"
                     "csrw mtvec, t0"
                     :
                     :
                     : "t0", "memory");
    __asm__ volatile("la t0, mtg_strap_entry\n\t"
                     "csrw stvec, t0"
                     :
                     :
                     : "t0", "memory");
    __asm__ volatile("la t0, mtg_st_regs\n\t"
                     "csrw sscratch, t0"
                     :
                     :
                     : "t0", "memory");
    ...
    // 4. mret with MPP=01 (S-mode) into mtg_smode_test. The whole
    // experiment runs in S-mode from there.
    __asm__ volatile("la t0, mtg_smode_test\n\t"
                     "csrw mepc, t0\n\t"
                     "csrr t0, mstatus\n\t"
