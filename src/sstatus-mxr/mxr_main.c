// mxr_main.c: sstatus.MXR as the S-mode gate on loads from
// execute-only pages (proof-backlog item "sstatus-mxr-probe").
//
// The mechanism under test: sstatus.MXR (bit 19) controls whether
...
// a load page fault (scause 0xd, stval = the faulting VA, sepc = the
// faulting load); with MXR=1 the same load must succeed and return
// the page contents.
//
// Phase A (MXR=0): one ld from the X-only VA 0x40000000. The
// delegated S-mode handler records scause/stval/sepc, advances sepc
// by 4 past the faulting ld, and srets. The ld's destination t0 is
// poisoned to 0 immediately before the load, so the value stored
// after a trap proves the load never completed.
// Phase B (MXR=1): csrs sstatus, MXR, then the same ld must return
// the canary word with no new trap.
//
...
mxr_save_t mxr_save;
volatile unsigned long mxr_trap_count;
volatile unsigned long mxr_scause;
volatile unsigned long mxr_stval;
volatile unsigned long mxr_sepc;
volatile unsigned long mxr_bad_seen;
volatile unsigned long mxr_bad_scause;
volatile unsigned long mxr_bad_sepc;

static unsigned long m_scratch_area[4];

// Page-table pages and the one mapped X-only data page, each a full
// 4 KiB page so the PPN arithmetic is exact. BSS clearing zeroes
// every other entry (invalid), which is what the walk relies on.
...
static volatile unsigned long mxrs_load_result;
...
// outcome, the trap counts, the MXR readbacks in both phases, the
...
static unsigned long measured_checksum(unsigned long mxr_phase_a,
                                       unsigned long mxr_phase_b,
                                       unsigned long load_result_a) {
    unsigned long h = 0xcbf29ce484222325UL;
    h = fnv1a_64(h, mxr_scause);
    h = fnv1a_64(h, mxr_stval);
    h = fnv1a_64(h, mxr_sepc);
    h = fnv1a_64(h, mxr_trap_count);
    h = fnv1a_64(h, mxr_bad_seen);
    h = fnv1a_64(h, mxr_phase_a);
    h = fnv1a_64(h, mxr_phase_b);
    h = fnv1a_64(h, load_result_a);
    h = fnv1a_64(h, mxrs_load_result);
...
void s_main(void) {
    unsigned long mxr_phase_a, mxr_phase_b, load_result_a, csum;

    uart_puts("s-mode: entered, translation active\n");

    // The gate under test starts closed, explicitly.
    __asm__ volatile("csrc sstatus, %0" :: "r"(SSTATUS_MXR));
    mxr_phase_a = (read_sstatus() & SSTATUS_MXR) != 0;
    uart_puts("phase A: sstatus.MXR=");
    uart_put_dec(mxr_phase_a);
    uart_puts(" (expect 0)\n");
    check(mxr_phase_a == 0, "MXR not clear at phase A");
    ...
    // would write it: mxrs_load_result == 0 proves the fault.
    mxrs_load_result = ~0UL;
    phase_a_resumed = 0;
    __asm__ volatile(
        "li t0, 0\n"                 // poison: proves the ld never completes
        ".global mxrs_fault_load\n"
        "mxrs_fault_load:\n"
        "ld t0, 0(%1)\n"             // faults here in phase A (MXR=0, X-only page)
        "sd t0, 0(%0)\n"             // reached only if the ld completes
        "1:\n"
        "li t0, 1\n"
        "sd t0, 0(%2)\n"             // phase_a_resumed = 1
        :
        : "r"(&mxrs_load_result), "r"(XPAGE_VA), "r"(&phase_a_resumed)
        : "t0", "memory");

    uart_puts("phase A: traps=");
    uart_put_dec(mxr_trap_count);
    uart_puts(" (expect 1) scause=");
    uart_put_hex(mxr_scause);
    uart_puts(" (expect 0xd) stval=");
    uart_put_hex(mxr_stval);
    uart_puts(" (expect 0x40000000) sepc=");
    uart_put_hex(mxr_sepc);
    uart_puts("\nfault load at mxrs_fault_load=");
    uart_put_hex((unsigned long)mxrs_fault_load);
    uart_puts(" load_result=");
    uart_put_hex(mxrs_load_result);
    uart_puts(" (expect 0) resumed=");
    uart_put_dec(phase_a_resumed);
    uart_puts(" (expect 1)\n");
    load_result_a = mxrs_load_result;
    check(mxr_trap_count == 1, "phase A did not produce exactly one trap");
    check(mxr_scause == SCAUSE_LPF, "scause != 0xd (load page fault)");
    check(mxr_stval == XPAGE_VA, "stval != faulting VA");
    check(mxr_sepc == (unsigned long)mxrs_fault_load,
    ...
    check(mxr_bad_seen == 0, "unexpected extra trap in phase A");

    // Phase B: open the gate, then the same load must return the
    // canary with no new trap.
    __asm__ volatile("csrs sstatus, %0" :: "r"(SSTATUS_MXR));
    mxr_phase_b = (read_sstatus() & SSTATUS_MXR) != 0;
    uart_puts("phase B: sstatus.MXR=");
    uart_put_dec(mxr_phase_b);
    uart_puts(" (expect 1)\n");
    check(mxr_phase_b == 1, "MXR not set in phase B");
    ...
    __asm__ volatile(
        ...
        "sd t0, 0(%0)\n"
        :
        : "r"(&mxrs_load_result), "r"(XPAGE_VA)
        : "t0", "memory");

    uart_puts("phase B: load_result=");
    uart_put_hex(mxrs_load_result);
    uart_puts(" (expect canary 0xe4ec0d1edeadbeef) traps=");
    uart_put_dec(mxr_trap_count);
    uart_puts(" (expect 1)\n");
    check(mxrs_load_result == CANARY,
          "phase-B load did not return the canary");
    check(mxr_trap_count == 1, "unexpected trap in phase B");
    check(mxr_bad_seen == 0, "unexpected extra trap in phase B");

    csum = measured_checksum(mxr_phase_a, mxr_phase_b, load_result_a);
...
int main(void) {
    ...
    uart_init();
    uart_puts("sstatus-mxr: sstatus.MXR gate on X-only page loads (proof-backlog item \"sstatus-mxr-probe\")\n");
    ...
    *(volatile unsigned long *)page = CANARY;
    ...
    uart_puts("control: canary 0xe4ec0d1edeadbeef stored in the X-only page: ok\n");

    // Scaffolding checks: the S-mode phase needs its code, stack,
    // tables, and the X-only page reachable, and the UART page
    // mapped.
    ...
    //   0x40000000 with X=1 only (V,A,D set; no R, no W, no U).
    ...
    check((l0_test[0] & (PTE_V | PTE_X)) == (PTE_V | PTE_X),
          "X-only leaf missing V/X");
    check((l0_test[0] & (PTE_R | PTE_W | PTE_U)) == 0,
          "X-only leaf has unexpected R/W/U bits");
    ...
    check(((l0_test[0] >> 10) & 0xFFFFFFFFFFFUL) == page_ppn,
          "X-only leaf PPN != canary page number");
    ...
    __asm__ volatile("csrw stvec, %0" :: "r"(mxr_trap_entry));
    __asm__ volatile("csrw sscratch, %0" :: "r"(&mxr_save));
