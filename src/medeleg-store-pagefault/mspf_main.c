// mspf_main.c: medeleg bit-15 store-page-fault trap destination
// switch (backlog item "riscv medeleg-store-pagefault").
//
// Exactly one mechanism is under test: the destination of a store
// page fault raised in S-mode as medeleg bit 15 flips. Phase A
// runs with medeleg bit 15 set: an S-mode store to an unmapped
// page must trap in S-mode with scause = 15 (store/AMO page fault),
// sepc at the faulting store instruction, and stval holding the
// faulting data address, with zero M-mode traps from the fault
// itself. Phase B runs with medeleg bit 15 clear: the same store
// must trap in M-mode with mcause = 15, mepc at the faulting store,
// and mtval holding the faulting data address, with the S-mode trap
// count unchanged.
//
...
// (invalid), so the walk dies at the leaf lookup for a store to
...
//   build the page tables, enable Sv39, write medeleg bit 15 and
//   read back the actual configuration (bit 15 verified set), arm
//   the phase-2 continuation in m_regs[7], then drop to S-mode.
//   S-mode phase A: store the resume address, record the store
//   instruction address, store a word to the fault page. The store
//   page fault is delegated, so the S-mode handler records
...
//   medeleg bit 15 (readback verified), arms the report
//   continuation, and drops to S-mode.
//   S-mode phase B: record the store instruction address, store a
//   word to the fault page. The fault is not delegated, so the
//   M-mode handler records mcause/mepc/mstatus/mtval and redirects
//   to the M-mode report, which prints every measured value, runs
//   the checks, takes a quiet window, and prints the verdict. All
//   interrupt enables stay clear for the whole run, so no
//   interrupt of either kind can fire.

#include "../uart.h"

#define BIT_STORE_PAGE_FAULT 15UL
...
// handler and the faulting store's own address, then store a word
// to the unmapped fault page. medeleg bit 15 is set, so the store
...
// equal it exactly while sepc must equal the store instruction's
...
void smode_phaseA(void) {
    __asm__ volatile(
        ...
        "li t1, 0xdeadbeef\n"
        "0:\n"
        "sd t1, 0(t0)\n"      // store to the unmapped page: traps
        "2:\n"
        "ecall\n"             // back to M-mode; cause 9, not delegated
        :: "r"(s_regs), "r"(&pA_store_pc), "i"(FAULT_VA)
        : "t0", "t1", "memory");
...
// continuation, clears medeleg bit 15 (readback verified), arms the
...
void phase2_mmode(void) {
    ...
    csr_write_medeleg(csr_read_medeleg() & ~(1UL << BIT_STORE_PAGE_FAULT));
    pB_medeleg_rb = csr_read_medeleg();

    drop_to_smode(smode_phaseB);
}

// Phase-B S-mode payload: record the faulting store's own address,
// then store a word to the fault page. medeleg bit 15 is clear,
// so the store page fault lands in the M-mode handler, which records
...
// mepc here after the phase-B store page fault. Prints every
...
void report_mmode(void) {
    ...
    uart_puts("medeleg-store-pagefault: store-page-fault trap destination switch test\n");
    ...
    uart_puts("phaseA: medeleg bit15 set readback=");
    uart_put_hex(pA_medeleg_rb);
    uart_puts("\n");
    uart_puts("phaseB: medeleg bit15 clear readback=");
    ...
    // Phase-A checks: medeleg bit 15 set, the store page fault
    // trapped in S-mode with the faulting data address in stval
    // and the store instruction's address in sepc (they differ,
    // which is what distinguishes a store page fault from an
    // instruction page fault), and the M-mode handler saw only
    // the ecall.
    check((pA_medeleg_rb & (1UL << BIT_STORE_PAGE_FAULT)) != 0,
          "phase-A medeleg readback missing bit 15");
    check(s_regs[0] == 1, "phase-A S-mode trap did not fire exactly once");
    check(s_regs[2] == CAUSE_STORE_PAGE_FAULT,
          "phase-A scause != 15 (store page fault)");
    ...
    // Phase-B checks: medeleg bit 15 clear, the store page fault
    // trapped in M-mode with the faulting data address in mtval
    // and the store instruction's address in mepc, and no S-mode
    // trap fired.
    check((pB_medeleg_rb & (1UL << BIT_STORE_PAGE_FAULT)) == 0,
          "phase-B medeleg readback still has bit 15 set");
    check(m_regs[0] == 2, "phase-B M-mode trap did not fire exactly once more");
    check(m_regs[2] == CAUSE_STORE_PAGE_FAULT,
          "phase-B mcause != 15 (store page fault)");
    ...
    check(s_regs[0] == 1,
          "S-mode trap fired in phase B with bit 15 clear");
...
int main(void) {
    ...
    uart_init();
    uart_puts("medeleg-store-pagefault: medeleg bit-15 store-page-fault trap destination switch\n");
    ...
    // Phase A: delegate store page faults to S-mode and read back
    // the actual configuration the first fault runs under.
    csr_write_medeleg(1UL << BIT_STORE_PAGE_FAULT);
    pA_medeleg_rb = csr_read_medeleg();
    uart_puts("m-mode: phase-A medeleg write=0x8000 readback=");
    uart_put_hex(pA_medeleg_rb);
    uart_puts("\n");
    ...
    uart_puts("m-mode: entering S-mode (phase A, bit 15 set)\n");
