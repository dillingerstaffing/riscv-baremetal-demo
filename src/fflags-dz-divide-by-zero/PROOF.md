...
Checks: 14
Mismatches: 0
Checksum: 0xc2e49a3a15e82680
Environment: QEMU 8.2.2
Verdict: PASS
-->

# PROOF: fflags DZ (divide-by-zero) accrual on fdiv.d 1.0/0.0 (backlog item "riscv fflags-dz-divide-by-zero")

Backlog item "riscv fflags-dz-divide-by-zero": in M-mode on QEMU,
clear `fflags`, run `fdiv.d` 1.0/0.0, and verify the DZ bit is set
with NX, UF, OF, NV all clear; publish the fcsr read triples
across 3 runs. Distinct from the done `src/fflags-nx-inexact`,
`src/fflags-uf-underflow`, and `src/fflags-of-overflow` modules,
which probe the NX, UF, and OF accrued flags respectively: each
probes a different accrued flag.

## What was built

`src/fflags-dz-divide-by-zero/`, a bare-metal M-mode binary sharing
only `src/boot.S` and the UART driver with the other demos. It
walks this sequence and checks every step in code:

1. Installs a counting M-mode trap handler (`fdz_trap.S`, direct
...
6. Loads the bit patterns for 1.0 (`0x3FF0000000000000`) and +0.0
   (`0x0000000000000000`) into integer registers, moves them into
   ...
   `fcsr` readback to be exactly `0x08`: DZ set, NX/UF/OF/NV all
   0, frm still RNE.
7. Reads the quotient back with `fmv.x.d` and requires it to equal
   `0x7FF0000000000000` (+infinity, the 1.0/+0.0 result). This is
...
Build integration: `Makefile` gains `fflags-dz-divide-by-zero.elf`
and `run-fflags-dz-divide-by-zero`; the module is in `all` and
`clean`.
...
```
800003c0:  1a3100d3   fdiv.d  ft1,ft2,ft3,rne
```
...
verified against the NX module's shipped encoding decode before
shipping). One occurrence of this encoding exists in the ELF, and
no other FP arithmetic instruction is present anywhere in the
binary.
...
9. After `fdiv.d` 1.0/0.0, the fflags field (bits 4:0) reads
   exactly `0x8` (DZ set, NX/UF/OF/NV all 0).
...
byte-identical (md5 `9d83f7f38e0af7ac060d43ce4104cf1c` on all three
...
```
fflags-dz-divide-by-zero: fdiv.d 1.0/0.0 must set fflags.DZ
setup: mtvec=0x800001c8 mie=0x0
setup: misa=0x80000000001411ad
before: fcsr=0x0 fflags=0x0 frm=0
after : fcsr=0x8 fflags=0x8 frm=0
quotient bits=0x7ff0000000000000
cleared: fcsr=0x0 fflags=0x0 frm=0
traps: count=0 mcause=0x0 mepc=0x0 mtval=0x0
triple fcsr: before=0x0 after=0x8 cleared=0x0
checks=14 mismatches=0
checksum=0xc2e49a3a15e82680
RESULT: PASS
```

fcsr triple: `0x0` (cleared) -> `0x8` (after 1.0/0.0) ->
`0x0` (cleared again). DZ is fflags bit 3, so `0x8` is DZ set
with NX (bit 0), UF (bit 1), OF (bit 2), NV (bit 4) all clear.
...
`make fflags-dz-divide-by-zero.elf` output:

```
riscv64-unknown-elf-gcc -Wall -Wextra -O2 -ffreestanding -nostdlib -nostartfiles -no-pie -fno-pie -fno-pic -march=rv64imac_zicsr -mabi=lp64 -mcmodel=medany -c src/fflags-dz-divide-by-zero/fdz_trap.S -o src/fflags-dz-divide-by-zero/fdz_trap.o
riscv64-unknown-elf-gcc -Wall -Wextra -O2 -ffreestanding -nostdlib -nostartfiles -no-pie -fno-pie -fno-pic -march=rv64imac_zicsr -mabi=lp64 -mcmodel=medany -c src/fflags-dz-divide-by-zero/fdz_main.c -o src/fflags-dz-divide-by-zero/fdz_main.o
riscv64-unknown-elf-gcc -Wall -Wextra -O2 -ffreestanding -nostdlib -nostartfiles -no-pie -fno-pie -fno-pic -march=rv64imac_zicsr -mabi=lp64 -mcmodel=medany -T link.ld -o fflags-dz-divide-by-zero.elf src/boot.o src/uart.o src/fflags-dz-divide-by-zero/fdz_trap.o src/fflags-dz-divide-by-zero/fdz_main.o
ld: warning: fflags-dz-divide-by-zero.elf has a LOAD segment with RWX permissions
```
...
```
qemu-system-riscv64 -machine virt -nographic -bios none -kernel fflags-dz-divide-by-zero.elf
```
