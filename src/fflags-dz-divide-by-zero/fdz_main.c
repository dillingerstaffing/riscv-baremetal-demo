// fdz_main.c: fflags DZ (divide-by-zero) bit accrual on fdiv.d 1.0/0.0
// (backlog item "riscv fflags-dz-divide-by-zero").
//
// Mechanism under test: the fflags field of the fcsr CSR is the
// accrued-exception record of floating-point operations. The DZ
// bit (fflags bit 3) must be set by a finite-over-zero divide. The
// module runs in M-mode on QEMU 8.2.2 (virt machine) and walks
// this sequence:
//
...
#define FFLAGS_DZ  0x08UL         // fcsr bit 3: divide-by-zero
...
#define INF_BITS   0x7FF0000000000000UL  // +infinity as a double bit pattern
...
static volatile unsigned long fdz_trap[4];

extern void fdz_trap_entry(void);
...
// One volatile divide-by-zero, 1.0/0.0, then the quotient's bit
...
static unsigned long div10_bits(void) {
    unsigned long q;
    __asm__ volatile(
        ".option push\n\t"
        ".option arch, +d\n\t"
...
int main(void) {
    ...
    uart_init();
    uart_puts("fflags-dz-divide-by-zero: fdiv.d 1.0/0.0 must set fflags.DZ\n");

    // Safety net: counting M-mode trap handler. No trap is expected;
    // the run requires the counter to stay 0.
    __asm__ volatile("csrw mtvec, %0" :: "r"((unsigned long)fdz_trap_entry));
    __asm__ volatile("csrw mscratch, %0" :: "r"((unsigned long)fdz_trap));
    ...
    uart_put_hex(mie);
    uart_puts("\n");
    checks++; check((tv & ~3UL) == (unsigned long)fdz_trap_entry,
    ...
    // 4. One divide-by-zero, 1.0/0.0: fcsr must read back exactly
    // 0x08 (DZ set, NX/UF/OF/NV all clear, frm still RNE).
    q = div10_bits();
    ...
    uart_put_hex(q);
    uart_puts("\n");
    checks++; check((fcsr_after & FFLAGS_ALL) == FFLAGS_DZ,
                    "fflags != 0x8 (DZ) after fdiv.d 1.0/0.0");
    ...
    checks++; check(((fcsr_after >> 5) & 7UL) == FRM_RNE,
                    "frm != RNE after fdiv.d 1.0/0.0");

    // 5. Sanity anchor: the quotient is +infinity. Logged for the
    // record; the verdict rests on the fflags checks.
    checks++; check(q == INF_BITS,
                    "quotient bits != 0x7FF0000000000000 (+inf)");
    ...
    uart_put_dec(fdz_trap[3]);
    uart_puts(" mcause=");
    uart_put_hex(fdz_trap[0]);
    uart_puts(" mepc=");
    uart_put_hex(fdz_trap[1]);
    uart_puts(" mtval=");
    uart_put_hex(fdz_trap[2]);
    uart_puts("\n");
    checks++; check(fdz_trap[3] == 0, "trap handler fired during the run");
