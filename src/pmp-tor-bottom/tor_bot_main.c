// tor_bot_main.c: PMP TOR inclusive-bottom-bound test.
//
// Exactly one mechanism is under test: a TOR PMP entry matches the
// half-open range [pmpaddr[i-1], pmpaddr[i]), so an access exactly
// at the bottom bound still matches the entry. The probes are: an
// lbu at the bottom bound of the denied range traps with the load
// access fault; an lbu one byte below the bottom bound matches no
// locked entry and completes with no trap, returning the expected
// byte under M-mode default-allow.
//
// Two entries are programmed. Entry 0 is the TOR bottom bound:
// pmpaddr0 = 0x20000c00 (base = 0x80003000), TOR over [0, base)
// with full permissions, unlocked. Entry 1 is TOR over
// [base, base + 4 KiB) with no permissions and the lock bit set,
// i.e. pmpcfg0 = 0x880F (entry 0: TOR + R|W|X; entry 1: L + TOR,
// R = W = X = 0). A locked TOR deny cannot be entry 0: entry 0's
// TOR range bottom is fixed at address 0, so a locked deny there
// would deny the program's own code, the UART, and the test
// finisher, and no probe could run. The bottom bound under test is
// pmpaddr0, entry 1's bottom bound, hence entry 1 must be the
// locked entry.
//
...
// when at least one entry is locked, so the below-bound probe's
// clean completion is a direct measurement of that rule, not of an
// allow entry: entry 0's unlocked byte is cleared (csrw pmpcfg0, 0)
// right after programming, which also verifies the per-entry lock
// rule (entry 1's byte stays 0x88, entry 0's clears to 0x00), and
// every probe runs with only the locked entry programmed.
//
...
// the sentinel byte one byte below the bottom bound (outside the
// denied range) reads back bit-identical, proving the trap had no
// side effect on the adjacent byte.
//
...
//   T1: lbu at base (the inclusive bottom bound of the denied
//       range) traps exactly once, mcause = 0x5, mepc = faulting
//       lbu, mtval = base, destination keeps its poison.
//   T2: lbu at base - 1 (below the bottom bound) matches no entry,
//       completes with 0 traps, and returns the sentinel.
//
// A minimal M-mode trap entry (tor_bot_trap.S) records
...
extern void tor_bot_trap_entry(void);

// Trap save area, laid out for tor_bot_trap.S:
// [0]=total traps [1]=t1 [2]=mcause [3]=mepc [4]=mtval
// [5]=resume pc [6]=seen flag [7]=unused.
// mscratch points here while a test is armed.
volatile unsigned long tor_bot_save[8];

// 4 KiB denied region, 4 KiB aligned so pmpaddr0 = start >> 2 is
// exact. The below-bound byte at start - 1 is stamped directly:
// it sits in BSS padding just below the aligned array (the same
// placement the done src/pmp-tor module used for its boundary-1
// sentinel), and the layout guard below asserts the check
// counters and the trap save area do not alias it.
static volatile unsigned char region[4096] __attribute__((aligned(4096)));
...
#define SENTINEL 0x5aUL  // byte stamped one byte below the bottom bound
#define EXPECTED_BASE 0x80003000UL  // the bottom bound under test
...
// trapped probe must equal tor_bot_save[5] - 4.
static unsigned long probe_load_byte(unsigned long addr) {
    unsigned long got;
    __asm__ volatile(
        ".option norvc\n\t"
        "la t0, 1f\n\t"
        "sd t0, 40(%1)\n\t"    // tor_bot_save[5]: resume pc
        "sd zero, 48(%1)\n\t"  // tor_bot_save[6]: seen = 0
        ...
        ".option rvc"
        : "=r"(got)
        : "r"(tor_bot_save), "r"(POISON64), "r"(addr)
        : "t0", "t1", "memory");
    return got;
}
...
int main(void) {
    ...
    uart_init();
    uart_puts("pmp-tor-bottom: PMP TOR inclusive-bottom-bound test\n");

    // All traps to M-mode, no interrupts: the only traps possible
    // are the probes' own synchronous faults.
    __asm__ volatile("csrw mtvec, %0" :: "r"(tor_bot_trap_entry));
    __asm__ volatile("csrw mscratch, %0" :: "r"(tor_bot_save));
    ...
    // Layout guard, five parts. (a) The denied region must start on
    // a 4 KiB boundary so pmpaddr0 = start >> 2 is exact, and the
    // run pins the exact bottom bound under test, start ==
    // 0x80003000. (b) The trap handler's stores land in
    // tor_bot_save, which must not be the denied region: the check
    // requires the whole 8-word array to end at or below the
    // region base. (c) The check counters themselves
    // (zero-initialized, hence .bss) must not alias the probed
    // range or the below-bound sentinel byte at start - 1: the
    // linker is free to place them right after the region array,
    // and the sibling run once lost a counter to exactly that.
    // (d) The sentinel byte sits one byte below the bottom bound;
    // the live stack must stay 1 KiB clear of the region end so
    // nothing clobbers the expected value. All values are run-time
    // constants, identical every run.
    {
        ...
        uart_puts(" end=");
        uart_put_hex(end);
        uart_puts(" tor_bot_save=");
        uart_put_hex((unsigned long)tor_bot_save);
        uart_puts(" sp=");
        uart_put_hex(sp_now);
        uart_puts("\n");
        check((start & 0xFFFUL) == 0, "region start not 4 KiB aligned");
        fold((start & 0xFFFUL) == 0);
        check(start == EXPECTED_BASE, "region start != 0x80003000");
        fold(start == EXPECTED_BASE);
        check((unsigned long)tor_bot_save + sizeof(tor_bot_save) <= start,
              "tor_bot_save reaches the denied region");
        fold((unsigned long)tor_bot_save + sizeof(tor_bot_save) <= start);
        check((unsigned long)&checks < start - 1 || (unsigned long)&checks >= end,
              "checks counter aliases the probed range");
        fold((unsigned long)&checks < start - 1 || (unsigned long)&checks >= end);
        check((unsigned long)&fails < start - 1 || (unsigned long)&fails >= end,
              "fails counter aliases the probed range");
        fold((unsigned long)&fails < start - 1 || (unsigned long)&fails >= end);
        check(sp_now > end + 0x400UL,
              "live stack within 1 KiB of the region end");
    ...
    // Stamp the sentinel one byte below the bottom bound, outside
    // the region that will be denied, and read it back before
    // programming: T2 must verify a real value, not merely the
    // absence of a trap.
    mem_write8(start - 1, (unsigned char)SENTINEL);
    rb = mem_read8(start - 1);
    check(rb == SENTINEL, "below-bound sentinel not writable before PMP");
    fold(rb);
    uart_puts("control: sentinel 0x5a one byte below the bottom bound: ok\n");

    // Program the entries. Entry 0: TOR over [0, start) with full
    // permissions, unlocked. Entry 1: TOR over [start, end) with no
    // permissions, locked. pmpaddr0 = start >> 2 = 0x20000c00 is
    // the bottom bound under test; pmpaddr1 = end >> 2.
    pmpaddr0 = start >> 2;
    pmpaddr1 = end >> 2;
    ...
    uart_put_hex(rb);
    uart_puts(" (expect start>>2 = 0x20000c00)\n");
    check(rb == pmpaddr0, "pmpaddr0 readback != start >> 2");
    check(rb == 0x20000c00UL, "pmpaddr0 readback != 0x20000c00");
    fold(rb);
    ...
    // programmed, so the below-bound probe's clean completion is a
    ...
    // T1: lbu exactly at the bottom bound. The TOR range is
    // half-open with the bottom bound inclusive, so the access
    // matches the locked deny entry and traps exactly once.
    got = probe_load_byte(start);
    seen = tor_bot_save[6];
    cause = tor_bot_save[2];
    epc = tor_bot_save[3];
    tval = tor_bot_save[4];
    resume = tor_bot_save[5];
    uart_puts("t1 lbu base     : seen=");
    uart_put_dec(seen);
    uart_puts(" mcause=");
    uart_put_hex(cause);
    uart_puts(" mepc=");
    ...
    check(tval == start, "t1: mtval != the bottom bound");
    ...
    // sentinel byte one byte below the bottom bound, just outside
    // the denied range, reads back bit-identical.
    rb = mem_read8(start - 1);
    uart_puts("t1 neighbor     : sentinel=");
    uart_put_hex(rb);
    uart_puts("\n");
    check(rb == SENTINEL, "t1: below-bound sentinel changed by the trap");
    fold(rb == SENTINEL);

    // T2: lbu one byte below the bottom bound. No entry covers it
    // (entry 0's byte was cleared, entry 1 starts at the bottom
    // bound), so with a locked entry programmed, M-mode
    // default-allow completes the access with 0 traps and returns
    // the sentinel.
    got = probe_load_byte(start - 1);
    seen = tor_bot_save[6];
    cause = tor_bot_save[2];
    epc = tor_bot_save[3];
    tval = tor_bot_save[4];
    resume = tor_bot_save[5];
    uart_puts("t2 lbu base-1   : seen=");
    uart_put_dec(seen);
    uart_puts(" mcause=");
    uart_put_hex(cause);
    uart_puts(" mepc=");
    ...
    uart_put_hex(got);
    uart_puts("\n");
    check(seen == 0, "t2: unexpected trap below the bottom bound");
    fold(seen);
    ...
    uart_put_dec(q0);
    uart_puts(" after=");
    uart_put_dec(tor_bot_save[0]);
    uart_puts("\n");
    check(tor_bot_save[0] == q0, "quiet window saw an unexpected trap");
    fold(tor_bot_save[0] - q0);
