// mlpf_main.c: medeleg bit-13 load-page-fault trap destination
// switch (backlog item "riscv medeleg-load-pagefault").
//
// Exactly one mechanism is under test: the destination of a load
// page fault raised in S-mode as medeleg bit 13 flips. Phase A
// runs with medeleg bit 13 set: an S-mode load from an unmapped
// page must trap in S-mode with scause = 13 (load page fault),
// sepc at the faulting load instruction, and stval holding the
// faulting data address, with zero M-mode traps from the fault
// itself. Phase B runs with medeleg bit 13 clear: the same load
// must trap in M-mode with mcause = 13, mepc at the faulting load,
// and mtval holding the faulting data address, with the S-mode
...
// (invalid), so the walk dies at the leaf lookup for a load from
...
//   build the page tables, enable Sv39, write medeleg bit 13 and
//   read back the actual configuration (bit 13 verified set), arm
//   the phase-2 continuation in m_regs[7], then drop to S-mode.
//   S-mode phase A: store the resume address, record the load
//   instruction address, load from the fault page. The load page
//   fault is delegated, so the S-mode handler records
//   scause/sepc/sstatus/stval and resumes the payload, which
//   issues an ecall (cause 9, not delegated) to get back to
//   M-mode.
//   M-mode: the handler records the ecall and redirects mepc to
//   phase2_mmode, which snapshots the phase-A counters, clears
//   medeleg bit 13 (readback verified), arms the report
//   continuation, and drops to S-mode.
//   S-mode phase B: record the load instruction address, load from
//   the fault page. The fault is not delegated, so the M-mode
//   handler records mcause/mepc/mstatus/mtval and redirects to the
//   M-mode report, which prints every measured value, runs the
//   checks, takes a quiet window, and prints the verdict. All
//   interrupt enables stay clear for the whole run, so no
//   interrupt of either kind can fire.

#include "../uart.h"

#define BIT_LOAD_PAGE_FAULT 13UL
...
static unsigned long pA_load_pc;      // phase-A faulting load address
static unsigned long pB_load_pc;      // phase-B faulting load address
...
// handler and the faulting load's own address, then load a word
// from the unmapped fault page. medeleg bit 13 is set, so the load
// page fault lands in the S-mode handler, which resumes at label 2;
// the ecall then returns to M-mode (cause 9 is not delegated). The
// fault-site address is the FAULT_VA constant itself, so stval must
// equal it exactly while sepc must equal the load instruction's
// address (unlike an instruction page fault, the two differ).
void smode_phaseA(void) {
    __asm__ volatile(
        "la t1, 2f\n"
        "sd t1, 56(%0)\n"     // s_regs[7]: resume address
        "la t1, 0f\n"
        "sd t1, 0(%1)\n"      // pA_load_pc: faulting load address
        "li t0, %2\n"
        "0:\n"
        "ld t1, 0(t0)\n"      // load from the unmapped page: traps
        "2:\n"
        "ecall\n"             // back to M-mode; cause 9, not delegated
        :: "r"(s_regs), "r"(&pA_load_pc), "i"(FAULT_VA)
        : "t0", "t1", "memory");
...
// continuation, clears medeleg bit 13 (readback verified), arms the
...
void phase2_mmode(void) {
    ...
    csr_write_medeleg(csr_read_medeleg() & ~(1UL << BIT_LOAD_PAGE_FAULT));
    pB_medeleg_rb = csr_read_medeleg();

    drop_to_smode(smode_phaseB);
}

// Phase-B S-mode payload: record the faulting load's own address,
// then load a word from the fault page. medeleg bit 13 is clear,
// so the load page fault lands in the M-mode handler, which records
// it and redirects to report_mmode. Never returns.
void smode_phaseB(void) {
    __asm__ volatile(
        "la t1, 0f\n"
        "sd t1, 0(%0)\n"      // pB_load_pc: faulting load address
        "li t0, %1\n"
        "0:\n"
        "ld t1, 0(t0)\n"      // load from the unmapped page: traps
        :: "r"(&pB_load_pc), "i"(FAULT_VA)
        : "t0", "t1", "memory");
...
// mepc here after the phase-B load page fault. Prints every
...
void report_mmode(void) {
    ...
    uart_puts("medeleg-load-pagefault: load-page-fault trap destination switch test\n");
    ...
    uart_puts("phaseA: medeleg bit13 set readback=");
    uart_put_hex(pA_medeleg_rb);
    uart_puts("\n");
    uart_puts("phaseB: medeleg bit13 clear readback=");
    ...
    // Phase-A checks: medeleg bit 13 set, the load page fault
    // trapped in S-mode with the faulting data address in stval
    // and the load instruction's address in sepc (they differ,
    // which is what distinguishes a load page fault from an
    // instruction page fault), and the M-mode handler saw only
    // the ecall.
    check((pA_medeleg_rb & (1UL << BIT_LOAD_PAGE_FAULT)) != 0,
          "phase-A medeleg readback missing bit 13");
    check(s_regs[0] == 1, "phase-A S-mode trap did not fire exactly once");
    check(s_regs[2] == CAUSE_LOAD_PAGE_FAULT,
          "phase-A scause != 13 (load page fault)");
    check(s_regs[3] == pA_load_pc,
          "phase-A sepc != faulting load address");
    check(s_regs[8] == FAULT_VA, "phase-A stval != fault page address");
    check(s_regs[3] != s_regs[8],
          "phase-A sepc equals stval; a load fault must separate them");
    ...
    // Phase-B checks: medeleg bit 13 clear, the load page fault
    // trapped in M-mode with the faulting data address in mtval
    // and the load instruction's address in mepc, and no S-mode
    // trap fired.
    check((pB_medeleg_rb & (1UL << BIT_LOAD_PAGE_FAULT)) == 0,
          "phase-B medeleg readback still has bit 13 set");
    check(m_regs[0] == 2, "phase-B M-mode trap did not fire exactly once more");
    check(m_regs[2] == CAUSE_LOAD_PAGE_FAULT,
          "phase-B mcause != 13 (load page fault)");
    check(m_regs[3] == pB_load_pc,
          "phase-B mepc != faulting load address");
    ...
    check(s_regs[0] == 1,
          "S-mode trap fired in phase B with bit 13 clear");
...
int main(void) {
    ...
    uart_init();
    uart_puts("medeleg-load-pagefault: medeleg bit-13 load-page-fault trap destination switch\n");
    ...
    // Phase A: delegate load page faults to S-mode and read back
    // the actual configuration the first fault runs under.
    csr_write_medeleg(1UL << BIT_LOAD_PAGE_FAULT);
    pA_medeleg_rb = csr_read_medeleg();
    uart_puts("m-mode: phase-A medeleg write=0x2000 readback=");
    uart_put_hex(pA_medeleg_rb);
    uart_puts("\n");
    ...
    uart_puts("m-mode: entering S-mode (phase A, bit 13 set)\n");
