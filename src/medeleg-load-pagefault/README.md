# medeleg-load-pagefault

Checks where a load-page-fault trap raised in S-mode lands as
`medeleg` bit 13 flips, under QEMU 8.2.2 in M-mode with a
hand-built Sv39 page table. Sibling of the `medeleg-instr-pagefault`
module (bit 12): same tables, same two-phase structure, but the
S-mode payload issues a load from the unmapped page instead of
jumping to it.

M-mode builds the tables by hand: `root[2]` -> `l1_id[0]` ->
`l0_id[]`, identity-mapping `[0x80000000, 0x80080000)` with R|W|X
and leaving `l0_id[128]` zero (invalid), so a load from the fault
...
Phase A writes `medeleg` bit 13 (readback `0x2000`, bit 13 verified
set), drops to S-mode, and loads from the fault page: the trap must
land in S-mode with `scause = 0xd`, `sepc` exactly at the faulting
load instruction and `stval` exactly at the faulting data address
(the two differ, which is what distinguishes a load page fault from
...
redirects into the phase-B setup. Phase B clears bit 13 (readback
`0x0`), drops to S-mode, and loads from the same page: the trap
must land in M-mode with `mcause = 0xd`, `mepc` exactly at the
faulting load and `mtval` exactly at the fault page, with the
...
- `mlpf_main.c`: two-phase driver, hand-built Sv39 tables with
...
- `mlpf_trap.S`: M-mode and S-mode trap entries (record, count, and
  resume/redirect).
...
Build and run from the repo root (CROSS to taste):
    ...
    make medeleg-load-pagefault.elf
    make run-medeleg-load-pagefault
