...
write/readback behavior of the `menvcfg.CBIE` field (bits 5:4) in M-mode: each
legal value 0, 1, 2, 3 reads back exactly as written (write
(boot&~0x30)|(v<<4), readback equals it, no other bit disturbed). An all-ones
WARL probe reports the legalized readback honestly (CBIE field survives,
