// cbie_main.c: menvcfg.CBIE write/readback WARL probe (backlog item
// riscv menvcfg-cbie-write).
//
// Mechanism under test: menvcfg.CBIE (bits 5:4) is a WARL field that
// governs cache-block-invalidate extension behavior; a write of each
// legal value 0..3 must read back exactly what was written (or the
// implementation-legalized subset), and the boot value must be
// restorable exactly. The experiment writes CBIE=1, 2, 3 and then 0
...
// CBIE field survives (it is implemented), bits set at boot do not
...
// trapping instruction, so the minimal trap handler (cbie_trap.S)
...
extern void cbie_trap_entry(void);

// Trap record, written by cbie_trap.S: [0] parked t1,
// [1] mcause, [2] mepc, [3] mtval, [4] trap count.
volatile unsigned long cbie_save[8];

#define MENVCFG_CBIE_MASK (0x30UL) // bits 5:4

#define VIRT_TEST_FINISHER ((volatile unsigned int *)0x100000UL)
#define FINISHER_PASS 0x5555u

// FNV-1a (64-bit) over the verdict-relevant values in a fixed order:
// the boot value, the four CBIE write/readback pairs, the two
...
int main(void) {
    ...
    uart_init();
    uart_puts("\n========================================\n");
    uart_puts("menvcfg.CBIE (bits 5:4) write/readback WARL probe\n");
    uart_puts("========================================\n\n");

    // Trap vector: any trap parks the hart (cbie_trap.S).
    __asm__ volatile("la t0, cbie_trap_entry\n\t"
                     "csrw mtvec, t0"
                     :
                     :
                     : "t0", "memory");
    ...
    uart_put_hex(boot);
    uart_puts(" CBIE(bits5:4)=");
    uart_put_dec((boot >> 4) & 3UL);
    uart_puts("\n");
    cks_feed(boot);

    // 2-5. CBIE probes: for each value 1, 2, 3, then 0, write the
    // value into bits 5:4 while holding every other bit at its boot
    ...
    base = boot & ~MENVCFG_CBIE_MASK;
    for (v = 1; v <= 3; v++) {
        w = base | ((unsigned long)v << 4);
        write_menvcfg(w);
        rb = read_menvcfg();
        uart_puts("cbie=");
        ...
        if (v == 1) {
            check(rb == w,
                  "CBIE=1 probe: readback != write (field did not "
                  "stick exactly, or other bits changed)");
        } else if (v == 2) {
            check(rb == w,
                  "CBIE=2 probe: readback != write (field did not "
                  "stick exactly, or other bits changed)");
        } else {
            check(rb == w,
                  "CBIE=3 probe: readback != write (field did not "
                  "stick exactly, or other bits changed)");
        }
        cks_feed(rb);
    }
    ...
    write_menvcfg(w);
    rb = read_menvcfg();
    uart_puts("cbie=0: write=");
    uart_put_hex(w);
    uart_puts(" readback=");
    uart_put_hex(rb);
    uart_puts("\n");
    check(rb == w,
          "CBIE=0 probe: readback != write (field did not clear "
    ...
    // the CBIE field survives the write (it is implemented on this
    ...
    uart_put_hex(warl2);
    uart_puts("\n");
    check((warl1 & MENVCFG_CBIE_MASK) == MENVCFG_CBIE_MASK,
          "WARL probe: CBIE field did not read back all-set on "
          "all-ones write");
