...
# sstatus.SPP=0 on U-mode ecall module: its own binary sharing only
...
# complement of the sstatus-spp module (SPP=1 case). On PASS shuts
...
# hart instead.
SPUT_SRCS := src/boot.S src/uart.c \
             src/sstatus-spp-u-trap/sppu_trap.S src/sstatus-spp-u-trap/sppu_main.c
SPUT_OBJS := $(SPUT_SRCS:.c=.o)
SPUT_OBJS := $(SPUT_OBJS:.S=.o)

sstatus-spp-u-trap.elf: $(SPUT_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(SPUT_OBJS)

# Run the sstatus.SPP=0 U-mode ecall module under QEMU.
run-sstatus-spp-u-trap: sstatus-spp-u-trap.elf
	$(QEMU) -machine virt -nographic -bios none -kernel sstatus-spp-u-trap.elf
...
# sret-in-U-mode module: its own binary sharing only boot.S and the
# UART driver with the other demos. Phase 1 (control) sretes from
# M-mode with SPP=1 into an S-mode pad that executes sret in
# S-mode (must return with no trap) and ecalls back to M-mode;
# phase 2 sretes from M-mode with SPP=0 into a U-mode pad whose
# sret must trap to M-mode with mcause=2, mepc at the sret, and
# mstatus.MPP=0. The M-mode handler records both scripted traps
# and jumps to a C continuation on the second; the bench harness
# runs QEMU under timeout, so a FAIL parks the hart instead of
# touching the virt test-device finisher.
# NOTE: src/boot.S must stay first in STU_SRCS so _start lands at
# 0x80000000, the address QEMU's -kernel loader starts at.
STU_SRCS := src/boot.S src/uart.c \
            src/sret-to-umode-fault/stu_trap.S src/sret-to-umode-fault/stu_main.c
STU_OBJS := $(STU_SRCS:.c=.o)
STU_OBJS := $(STU_OBJS:.S=.o)

sret-to-umode-fault.elf: $(STU_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(STU_OBJS)

# Run the sret-in-U-mode module under QEMU.
run-sret-to-umode-fault: sret-to-umode-fault.elf
	$(QEMU) -machine virt -nographic -bios none -kernel sret-to-umode-fault.elf
...
# mstatus.TVM trap module (proof-backlog item "riscv mstatus-tvm-trap"):
# its own binary sharing only boot.S and the UART driver with the
# other demos. With mstatus.TVM set, an S-mode write to satp and an
# S-mode sfence.vma each raise an illegal-instruction trap in M-mode
# with mepc at the faulting site; with TVM clear the same two
# operations execute normally. On PASS it shuts the machine down via
# the virt test-device finisher so the QEMU process exit code (0)
# reflects the verdict; on FAIL it parks the hart instead.
TVT_SRCS := src/boot.S src/uart.c \
            src/mstatus-tvm-trap/tvt_trap.S src/mstatus-tvm-trap/tvt_main.c
TVT_OBJS := $(TVT_SRCS:.c=.o)
TVT_OBJS := $(TVT_OBJS:.S=.o)

mstatus-tvm-trap.elf: $(TVT_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(TVT_OBJS)

# Run the mstatus.TVM trap module under QEMU.
run-mstatus-tvm-trap: mstatus-tvm-trap.elf
	$(QEMU) -machine virt -nographic -bios none -kernel mstatus-tvm-trap.elf

# mstatus.TSR trap module (proof-backlog item "riscv mstatus-tsr-trap"):
# its own binary sharing only boot.S and the UART driver with the
# other demos. With mstatus.TSR set, an S-mode sret raises an
# illegal-instruction trap in M-mode with mepc exactly at the sret;
# with TSR clear the same sret returns to S-mode with no trap. On
# PASS it shuts the machine down via the virt test-device finisher
# so the QEMU process exit code (0) reflects the verdict; on FAIL
# it parks the hart instead.
TSRT_SRCS := src/boot.S src/uart.c \
            src/mstatus-tsr-trap/tsr_trap.S src/mstatus-tsr-trap/tsr_main.c
TSRT_OBJS := $(TSRT_SRCS:.c=.o)
TSRT_OBJS := $(TSRT_OBJS:.S=.o)

mstatus-tsr-trap.elf: $(TSRT_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(TSRT_OBJS)

# Run the mstatus.TSR trap module under QEMU.
run-mstatus-tsr-trap: mstatus-tsr-trap.elf
	$(QEMU) -machine virt -nographic -bios none -kernel mstatus-tsr-trap.elf

# mstatus.MPRV load-privilege module (proof-backlog item "riscv
# mstatus-mprv-load"): its own binary sharing only boot.S and the
# UART driver with the other demos. Installs one unlocked PMP TOR
# entry over a 4 KiB test page with R=W=X=0 (M-mode loads bypass the
# unlocked entry; the same page checked as U-mode faults). Phase A:
# MPRV=0, MPP=U, the M-mode load must succeed. Phase B: MPRV=1,
# MPP=U, the same load must raise a load access fault (mcause=0x5)
# with mepc exactly at the probe load site and mtval equal to the
# faulting address; the handler records mcause/mepc/mtval and skips
# the 4-byte load. Phase C: MPRV=1, MPP=M, the load must succeed
# again. Phase D rewrites the PMP entry, verifies the load succeeds
# with MPRV=0, zeroes the PMP entry back to the boot state, and
# restores the boot mstatus bit-for-bit. On PASS it shuts the
# machine down via the virt test-device finisher so the QEMU process
# exit code (0) reflects the verdict; on FAIL it parks the hart
# instead.
MPRVL_SRCS := src/boot.S src/uart.c \
            src/mstatus-mprv-load/mprv_trap.S src/mstatus-mprv-load/mprv_main.c
MPRVL_OBJS := $(MPRVL_SRCS:.c=.o)
MPRVL_OBJS := $(MPRVL_OBJS:.S=.o)

mstatus-mprv-load.elf: $(MPRVL_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(MPRVL_OBJS)

# Run the mstatus.MPRV load-privilege module under QEMU.
run-mstatus-mprv-load: mstatus-mprv-load.elf
	$(QEMU) -machine virt -nographic -bios none -kernel mstatus-mprv-load.elf
...
# medeleg bit-12 instruction-page-fault trap destination switch module
# (backlog item "riscv medeleg-instr-pagefault"): its own binary
# sharing only boot.S and the UART driver with the other demos.
# M-mode builds a minimal Sv39 table by hand: root[2] -> l1_id[0] ->
# l0_id[], identity-mapping [0x80000000, 0x80080000) with R|W|X and
# leaving l0_id[128] zero (invalid), so a fetch from the fault page
# at 0x80080000 dies at the leaf lookup. One PMP NAPOT entry grants
# S-mode R|W|X over the whole address space; Sv39 is enabled via
# satp + sfence.vma. Phase A runs with medeleg bit 12 set: an
# S-mode jump to the fault page must trap in S-mode with scause = 12
# (instruction page fault), sepc at the faulting fetch address, and
# stval holding the faulting address, with the M-mode handler seeing
# only the phase-A ecall. Phase B runs with medeleg bit 12 clear:
# the same jump must trap in M-mode with mcause = 12 and mepc at
# the faulting address, with the S-mode trap count unchanged.
# Per-mode trap counts, causes, PCs, trap values, the medeleg
# write/readback values, and an FNV-1a checksum over the verdict
# values are printed; all interrupt enables stay clear, so no
# interrupt can fire in either phase. PASS writes 0x5555 to the
# virt test-device finisher so the QEMU exit code (0) reflects the
# verdict; on FAIL it parks the hart.
# NOTE: src/boot.S must stay first in MIPF_SRCS so _start lands at
# 0x80000000, the address QEMU's -kernel loader starts at.
MIPF_SRCS := src/boot.S src/uart.c \
             src/medeleg-instr-pagefault/mipf_trap.S src/medeleg-instr-pagefault/mipf_main.c
MIPF_OBJS := $(MIPF_SRCS:.c=.o)
MIPF_OBJS := $(MIPF_OBJS:.S=.o)

medeleg-instr-pagefault.elf: $(MIPF_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(MIPF_OBJS)

# Run the medeleg instruction-page-fault routing module under QEMU.
run-medeleg-instr-pagefault: medeleg-instr-pagefault.elf
	$(QEMU) -machine virt -nographic -bios none -kernel medeleg-instr-pagefault.elf

# medeleg bit-13 load-page-fault trap destination switch module
# (backlog item "riscv medeleg-load-pagefault"): its own binary
# sharing only boot.S and the UART driver with the other demos.
# Sibling of the bit-12 instruction-page-fault module: the same
# hand-built Sv39 table (root[2]->l1[0]->l0_id[], identity
# [0x80000000,0x80080000) R|W|X, l0_id[128] invalid) and the same
# two-phase structure, but the S-mode payload issues a load from
# the unmapped page instead of jumping to it. Phase A runs with
# medeleg bit 13 set (readback verified): the load must trap in
# S-mode with scause = 13, sepc at the faulting load and stval at
# the fault page, while M-mode sees only the phase-A return ecall.
# Phase B runs with bit 13 clear (readback 0x0): the same load must
# trap in M-mode with mcause = 13, mepc at the faulting load and
# mtval at the fault page, with the S-mode trap count unchanged.
# Per-mode trap counts, causes, PCs, trap values, the medeleg
# write/readback values, and an FNV-1a checksum over the verdict
# values are printed; all interrupt enables stay clear, so no
# interrupt can fire in either phase. PASS writes 0x5555 to the
# virt test-device finisher so the QEMU exit code (0) reflects the
# verdict; on FAIL it parks the hart.
# NOTE: src/boot.S must stay first in MLPF_SRCS so _start lands at
# 0x80000000, the address QEMU's -kernel loader starts at.
MLPF_SRCS := src/boot.S src/uart.c \
             src/medeleg-load-pagefault/mlpf_trap.S src/medeleg-load-pagefault/mlpf_main.c
MLPF_OBJS := $(MLPF_SRCS:.c=.o)
MLPF_OBJS := $(MLPF_OBJS:.S=.o)

medeleg-load-pagefault.elf: $(MLPF_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(MLPF_OBJS)

# Run the medeleg load-page-fault routing module under QEMU.
run-medeleg-load-pagefault: medeleg-load-pagefault.elf
	$(QEMU) -machine virt -nographic -bios none -kernel medeleg-load-pagefault.elf

# medeleg bit-15 store-page-fault trap destination switch module
# (backlog item "riscv medeleg-store-pagefault"): its own binary
# sharing only boot.S and the UART driver with the other demos.
# Sibling of the bit-12 instruction-page-fault and bit-13
# load-page-fault modules: the same hand-built Sv39 table
# (root[2]->l1[0]->l0_id[], identity [0x80000000,0x80080000) R|W|X,
# l0_id[128] invalid) and the same two-phase structure, but the
# S-mode payload issues a store to the unmapped page instead of a
# load. Phase A runs with medeleg bit 15 set (readback verified):
# the store must trap in S-mode with scause = 15, sepc at the
# faulting store and stval at the fault page, while M-mode sees only
# the phase-A return ecall. Phase B runs with bit 15 clear (readback
# 0x0): the same store must trap in M-mode with mcause = 15, mepc at
# the faulting store and mtval at the fault page, with the S-mode
# trap count unchanged. Per-mode trap counts, causes, PCs, trap
# values, the medeleg write/readback values, and an FNV-1a checksum
# over the verdict values are printed; all interrupt enables stay
# clear, so no interrupt can fire in either phase. PASS writes
# 0x5555 to the virt test-device finisher so the QEMU exit code (0)
# reflects the verdict; on FAIL it parks the hart.
# NOTE: src/boot.S must stay first in MSPF_SRCS so _start lands at
# 0x80000000, the address QEMU's -kernel loader starts at.
MSPF_SRCS := src/boot.S src/uart.c \
             src/medeleg-store-pagefault/mspf_trap.S src/medeleg-store-pagefault/mspf_main.c
MSPF_OBJS := $(MSPF_SRCS:.c=.o)
MSPF_OBJS := $(MSPF_OBJS:.S=.o)

medeleg-store-pagefault.elf: $(MSPF_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(MSPF_OBJS)

# Run the medeleg store-page-fault routing module under QEMU.
...
# sharing only boot.S and the UART driver with the other demos.
# Sibling of the bit-15 store-page-fault module: the same hand-built
# Sv39 table (root[2]->l1[0]->l0_id[], identity
# [0x80000000,0x80080000) R|W|X), but l0_id[128] is VALID R|W (no X)
# over the store page at 0x80080000, so translation succeeds and
# the fault comes from a locked PMP TOR deny entry covering
# exactly the store page. Three TOR entries, lowest-numbered match
# wins: entry 0 allows [0,0x80080000) R|W|X unlocked, entry 1 LOCKED
# denies [0x80080000,0x80081000), entry 2 allows
# [0x80081000,0x100000000) R|W|X unlocked; every pmpcfg/pmpaddr
# register is read back and verified. Phase A runs with medeleg
# bit 7 set (readback verified): the S-mode store to the denied
# page must trap in S-mode with scause = 7, sepc at the faulting
# store and stval at the store page, while M-mode sees only the
# phase-A return ecall. Phase B runs with bit 7 clear (readback
# 0x0): the same store must trap in M-mode with mcause = 7, mepc
# at the faulting store and mtval at the store page, with the
# S-mode trap count unchanged. Each phase first stores to an
# allowed scratch word (control, completes with no trap, proving
# the fault comes from the PMP entry), a quiet window moves
# neither trap counter, and the boot medeleg is restored and
# reported at the end. Per-mode trap counts, causes, PCs, trap
# values, the medeleg write/readback values, the PMP readbacks,
# and an FNV-1a checksum over the verdict values are printed; all
# interrupt enables stay clear, so no interrupt can fire in either
# phase. PASS writes 0x5555 to the virt test-device finisher so
# the QEMU exit code (0) reflects the verdict; on FAIL it parks
# the hart.
# NOTE: src/boot.S must stay first in MSAF_SRCS so _start lands at
# 0x80000000, the address QEMU's -kernel loader starts at.
MSAF_SRCS := src/boot.S src/uart.c \
             src/medeleg-store-access-fault/msaf_trap.S src/medeleg-store-access-fault/msaf_main.c
MSAF_OBJS := $(MSAF_SRCS:.c=.o)
MSAF_OBJS := $(MSAF_OBJS:.S=.o)

medeleg-store-access-fault.elf: $(MSAF_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(MSAF_OBJS)

# Run the medeleg store-access-fault routing module under QEMU.
run-medeleg-store-access-fault: medeleg-store-access-fault.elf
	$(QEMU) -machine virt -nographic -bios none -kernel medeleg-store-access-fault.elf

# pmp entry-priority first-match module (backlog item
# "riscv pmp-priority-first-match"): its own binary sharing only
# boot.S and the UART driver with the other demos. Two unlocked
# NAPOT entries over the same 4 KiB scratch page (same pmpaddr
# value) with conflicting permissions, plus a higher-numbered
# broad NAPOT R|W|X allow over [0x80000000,0x100000000) so S-mode
# code fetch and the control load keep working; every pmpcfg/
# pmpaddr register is read back and verified. medeleg stays 0 and
# satp stays Bare, so the probe load runs in S-mode against the
# physical page address. Phase A (entry 0 NAPOT R|W allow, entry 1
# NAPOT deny): the S-mode lbu of a canary completes with no trap
# and an ecall returns to M-mode. Phase B (pmpcfg0 rewritten with
# entry 0 deny, entry 1 allow): the same lbu traps in M-mode with
# mcause = 0x5, mepc at the faulting load and mtval at the scratch
# page. Each phase first loads a control byte covered only by the
# broad allow entry (completes in both phases), a quiet window
# moves neither trap counter, and the boot PMP config is restored
# and reported at the end. Per-phase pmpcfg/pmpaddr readbacks,
# trap counts, causes, PCs, trap values, and an FNV-1a checksum
# over the verdict values are printed; all interrupt enables stay
# clear, so no interrupt can fire in either phase. PASS writes
# 0x5555 to the virt test-device finisher so the QEMU exit code
# (0) reflects the verdict; on FAIL it parks the hart.
# NOTE: src/boot.S must stay first in PP_SRCS so _start lands at
# 0x80000000, the address QEMU's -kernel loader starts at.
PP_SRCS := src/boot.S src/uart.c \
           src/pmp-priority/pp_trap.S src/pmp-priority/pp_main.c
PP_OBJS := $(PP_SRCS:.c=.o)
PP_OBJS := $(PP_OBJS:.S=.o)

pmp-priority.elf: $(PP_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(PP_OBJS)

# Run the pmp entry-priority module under QEMU.
run-pmp-priority: pmp-priority.elf
	$(QEMU) -machine virt -nographic -bios none -kernel pmp-priority.elf
...
# mideleg bit-9 SEIP clear-and-restore module (backlog item
# "riscv mideleg-seip-clear"): its own binary sharing only boot.S
# and the UART driver with the other demos (plus the CLINT helper
# for the machine-timer disarm). Sibling of mideleg-seip-route,
# which verified the set path only. Phase A replicates the sibling
# as control: with bit 9 set, a pended SEI traps exactly once in
# S-mode. Phase B clears bit 9 and requires the next pended SEI to
# trap exactly once in M-mode with mcause=0x8000000000000009 (the
# SEI source keeps code 9; delegation selects the taker, not the
# code) and 0 new S-mode traps. Phase C re-sets bit 9 and requires
# Phase D clears the pending bit, runs a 2,000,000-cycle quiet
# window with the counters frozen, and restores mideleg and
# mstatus to their boot readbacks bit-for-bit. S-mode phases hand
# control back to M-mode via ecall (medeleg is 0, so the ecall
# traps in M-mode; the handler jumps to a trampoline target). On
# PASS it shuts the machine down via the virt test-device finisher
# so the QEMU process exit code (0) reflects the verdict; on FAIL
# it parks instead.
# NOTE: src/boot.S must stay first in SEIPCLR_SRCS so _start lands
# at 0x80000000, the address QEMU's -kernel loader starts at.
SEIPCLR_SRCS := src/boot.S src/uart.c src/preempt/clint.c \
             src/mideleg-seip-clear/seip_clear_trap.S src/mideleg-seip-clear/seip_clear_main.c
SEIPCLR_OBJS := $(SEIPCLR_SRCS:.c=.o)
SEIPCLR_OBJS := $(SEIPCLR_OBJS:.S=.o)

mideleg-seip-clear.elf: $(SEIPCLR_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(SEIPCLR_OBJS)
...
# fflags.NX isolation module (backlog "riscv fflags-nx-only"): its
# own binary sharing only boot.S and the UART driver with the
# other demos. Sets mstatus.FS to Initial (1) (an FP instruction
# with FS == Off would raise illegal-instruction), then runs two
# phases, each: csrw fcsr, 0x00 (frm=RNE, fflags cleared) with
# the readback required exactly 0x00, one volatile in-asm
# fadd.d with rm=DYN so the hardware rounds with the frm field,
# and result bits read back with fmv.x.d. Phase A is inexact:
# fadd.d(1e16, 1.0); the exact sum 10^16+1 is not
# representable (ulp 2 in the binade, exact sum halfway between
# the neighbors), so the readback must be exactly 0x01 (NX set,
# NV/DZ/OF/UF clear, frm still RNE) and the result bits must be
# the correctly rounded value 0x4341c37937e08000 (RNE
# ties-to-even picks the even significand, i.e. 1e16 itself).
# Phase B is the exact control: fadd.d(1.0, 2.0) = 3.0, whose
# readback must be exactly 0x00 (no accrued flag moved). fcsr is
# restored to its exact boot value at the end. A counting M-mode
# trap handler is installed as a safety net; the run requires
# its counter to stay 0, and a final FS readback sanity check
# requires FS != Off after the FP writes. On PASS it shuts the
# machine down via the virt test-device finisher so the QEMU
# process exit code (0) reflects the verdict; on FAIL it parks
# the hart instead.
# NOTE: src/boot.S must stay first in FNO_SRCS so _start lands at
# 0x80000000, the address QEMU's -kernel loader starts at.
FNO_SRCS := src/boot.S src/uart.c \
            src/fflags-nx-only/fnxo_trap.S src/fflags-nx-only/fnxo_main.c
FNO_OBJS := $(FNO_SRCS:.c=.o)
FNO_OBJS := $(FNO_OBJS:.S=.o)

fflags-nx-only.elf: $(FNO_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(FNO_OBJS)

# Run the fflags-nx-only module under QEMU.
run-fflags-nx-only: fflags-nx-only.elf
	$(QEMU) -machine virt -nographic -bios none -kernel fflags-nx-only.elf
...
# frm-rounding-static module (backlog "riscv frm-rounding-static"):
# its own binary sharing only boot.S and the UART driver with the
# other demos. Sets mstatus.FS to Initial (1) (an FP instruction
# with FS == Off would raise illegal-instruction), then runs four
# trials, each: csrw fcsr to set the frm field and clear fflags
# (0x00 for RNE, 0x40 for RDN), require the exact readback, one
# volatile fadd.d, then read the result bits (fmv.x.d) and fcsr.
# Trials 1 and 2 execute a hand-encoded .word (0x023120d3:
# fadd.d f1, f2, f3 with rm=010, RDN, built from the field table
# and cross-checked against GAS) while frm=RNE; on the
# exact-halfway pair 1.0 + 2^-53 and on 1.0 + 1.5*2^-53 the static
# mode must deliver the lower neighbor 0x3FF0000000000000. Control
# A runs the same 0.75-ulp pair with rm=111 (DYN) under frm=RNE
# and must deliver the upper neighbor 0x3FF0000000000001; the
# divergence proves the static result came from the instruction
# word, not frm. Control B runs rm=111 under frm=RDN on the
# halfway pair and must match the static bits exactly. Every
# trial asserts the result bits against the analytically derived
# expectation, fflags == 0x01 (NX only), and the frm field still
# reading the trial's mode. fcsr is restored to its exact boot
# value at the end. A counting M-mode trap handler is installed
# as a safety net; the run requires its counter to stay 0, and a
# final FS readback sanity check requires FS != Off after the FP
# writes. On PASS it shuts the machine down via the virt
# test-device finisher so the QEMU process exit code (0) reflects
# the verdict; on FAIL it parks the hart instead.
# NOTE: src/boot.S must stay first in FRS_SRCS so _start lands at
# 0x80000000, the address QEMU's -kernel loader starts at.
FRS_SRCS := src/boot.S src/uart.c \
            src/frm-rounding-static/frs_trap.S src/frm-rounding-static/frs_main.c
FRS_OBJS := $(FRS_SRCS:.c=.o)
FRS_OBJS := $(FRS_OBJS:.S=.o)

frm-rounding-static.elf: $(FRS_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(FRS_OBJS)

# Run the frm-rounding-static module under QEMU.
run-frm-rounding-static: frm-rounding-static.elf
	$(QEMU) -machine virt -nographic -bios none -kernel frm-rounding-static.elf

# PMP NAPOT match-behavior module (backlog item "riscv
# pmp-napot-match"): its own binary sharing only boot.S and the
# UART driver with the other demos. Programs pmpaddr0 = 0x200009ff
# (4 KiB NAPOT at 0x80002000) with pmpcfg0 entry 0 = 0x18
# (A=NAPOT, L=0, R=W=X=0: an unlocked deny entry) and proves the
# match semantics: the entry is not checked against M-mode
# accesses (it is unlocked), so each probe runs with
# mstatus.MPRV=1 and MPP=S, making the access privilege-checked
# as S-mode. A lw at 0x80002000 and at 0x80002ffc (first/last
# word inside) must each trap exactly once with mcause=0x5, mepc
# at the faulting lw (resume-4), mtval the faulting address, and
# the destination keeping its poison value; a lw at 0x80001ffc
# and at 0x80003000 (one word outside each edge) must complete
# with 0 traps and return the canary. A sw at 0x80002000 must
# trap once with mcause=0x7; sw at 0x80001ffc/0x80003000 must
# complete and round-trip. A minimal M-mode trap entry records
# mcause/mepc/mtval and counts every entry; a quiet window of
# ordinary M-mode work with the entry still programmed must show
# 0 new traps. Boot pmpaddr0/pmpcfg0/mstatus are restored exactly
# at the end, and the restore is checked. On PASS the machine
...
# NOTE: src/boot.S must stay first in PNM_SRCS so _start lands at
# 0x80000000, the address QEMU's -kernel loader starts at.
PNM_SRCS := src/boot.S src/uart.c \
            src/pmp-napot-match/pnm_trap.S src/pmp-napot-match/pnm_main.c
PNM_OBJS := $(PNM_SRCS:.c=.o)
PNM_OBJS := $(PNM_OBJS:.S=.o)

pmp-napot-match.elf: $(PNM_OBJS) link.ld
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $(PNM_OBJS)
