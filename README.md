...
- **src/menvcfg-cbze-write/**: M-mode WARL probe of menvcfg.CBZE (bits 7:6): writes CBZE values 1, 2, 3, 0 and verifies each reads back exactly, writes all-ones twice and publishes the legalized readback 0xa0000000000000f1 with the CBZE field sticking, restores the boot value 0x2000000000000000 bit-for-bit, 9 checks, 0 mismatches, checksum 0x5073d6e9f997f3a3 byte-identical across 3 QEMU 8.2.2 runs, 0 traps. [src/menvcfg-cbze-write/](src/menvcfg-cbze-write/)
- **src/fcsr-frm-rounddn/**: proves frm=RDN steers rounding of an inexact fsub.d toward negative infinity: the fcsr write 0x40 reads back exactly, the RDN result is exactly 1 ulp below the RNE result, NX sets and no other fflag moves, 20 checks, 0 mismatches, checksum 0xa92c23f3ba9e7afc byte-identical across 3 QEMU 8.2.2 runs, 0 traps. [src/fcsr-frm-rounddn/](src/fcsr-frm-rounddn/)
