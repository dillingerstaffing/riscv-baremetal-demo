...
- sstatus-spp-u-trap: sstatus.SPP record on a delegated U-mode ecall trap; U-mode ecalls delegated via medeleg bit 8, the S-mode handler records SPP=0, scause 0x8 and sepc at the ecall site, 0 M-mode traps, 8 checks, 0 mismatches, 3 byte-identical QEMU 8.2.2 runs PASS (see `src/sstatus-spp-u-trap/`)
...
- mie-write-readback: mie WARL write/readback probe in M-mode; writes all-ones to mie and publishes the legalized readback, probes bits 0..11 individually, and restores the boot value exactly, with mstatus.MIE clear throughout (see `src/mie-write-readback/`)
...
- **medeleg breakpoint route** (see `src/medeleg-breakpoint/`):
  ...
  runs, Verdict PASS.
- **medeleg instruction-page-fault route** (see `src/medeleg-instr-pagefault/`):
  medeleg bit 12 selects the trap destination for instruction page
...
- **src/stvec-vectored-mode/**: stvec vectored mode entry split measured both ways: a delegated illegal-instruction exception enters at BASE while a delegated supervisor software interrupt enters at BASE+4, with each stub recording its own hardware entry pc. [src/stvec-vectored-mode/](src/stvec-vectored-mode/)
...
- **src/fflags-nx-only/**: proves fflags.NX is the accrued inexact flag: one inexact fadd.d (1e16 + 1.0) sets NX and moves no other accrued flag, while an exact fadd.d (1.0 + 2.0) moves nothing. [src/fflags-nx-only/](src/fflags-nx-only/)
- **src/fflags-uf-underflow/**: clears fflags, executes fmul.d DBL_MIN * DBL_MIN, and verifies the UF (underflow) bit sets together with NX while no other accrued flag moves. [src/fflags-uf-underflow/](src/fflags-uf-underflow/)
...
- **src/fflags-of-overflow/**: clears fflags, executes fmul.d DBL_MAX * DBL_MAX, and verifies the OF (overflow) bit sets together with NX while no other accrued flag moves. [src/fflags-of-overflow/](src/fflags-of-overflow/)
- **src/fflags-dz-divide-by-zero/**: clears fflags, executes fdiv.d 1.0/0.0, and verifies the DZ (divide-by-zero) bit sets while no other accrued flag moves, with the quotient reading +inf. [src/fflags-dz-divide-by-zero/](src/fflags-dz-divide-by-zero/)
- **src/frm-rdn-vs-rup-div/**: executes fdiv.d 1.0/3.0 under frm=RDN and frm=RUP, and verifies the two quotients are adjacent doubles exactly 1 ulp apart. [src/frm-rdn-vs-rup-div/](src/frm-rdn-vs-rup-div/)
- **src/cbo-zero-gate/**: menvcfg.CBZE gate probe for Zicboz cbo.zero; the phase-0 probe found this QEMU 8.2.2 build raises illegal instruction on cbo.zero, so the module ships the honest probe-only slice with the full gate test implemented for when the emulator executes the instruction. [src/cbo-zero-gate/](src/cbo-zero-gate/)
- **src/sstatus-fs-off-fp-trap/**: executes fadd.d with mstatus.FS=Off and verifies the hart traps illegal-instruction with mepc at the fault site, then runs fadd.d 1.5+2.25 with FS=Initial and verifies f0 reads back the exact 3.75 with no new trap. [src/sstatus-fs-off-fp-trap/](src/sstatus-fs-off-fp-trap/)
- **src/mtimecmp-delta-tracks-mtime/**: reads the CLINT mtime twice around a delay and verifies the free-running counter never moves backward, with strictly positive deltas across the delay. [src/mtimecmp-delta-tracks-mtime/](src/mtimecmp-delta-tracks-mtime/)
- **src/mtimecmp-rw/**: writes three patterns (0x0, 0x123456789ABCDEF0, all-ones) to the CLINT mtimecmp register and verifies each reads back exactly, with all mie bits clear so the pending MTIP after the 0x0 write cannot be delivered and the trap count stays zero. [src/mtimecmp-rw/](src/mtimecmp-rw/)
* src/sstatus-fs-off-fp-trap: M-mode FS=Off illegal-instruction probe on fadd.d (fadd.d = floating add double). Commit 8c2020c / dir https://github.com/dillingerstaffing/riscv-baremetal-demo/tree/main/src/sstatus-fs-off-fp-trap
- **src/menvcfg-pbmte-write/**: M-mode WARL probe of menvcfg.PBMTE bit 62; writes 1 and verifies it reads back 0 on QEMU 8.2.2. [src/menvcfg-pbmte-write/](src/menvcfg-pbmte-write/)
- **src/mip-msip-write/**: attempts software writes of mip.MSIP (bit 3) in M-mode and verifies this QEMU ignores them: a control write of mip.SSIP (bit 1) via csrsi/csrci sticks (readback 0x82, then 0x80), while csrsi/csrci of bit 3 leave mip at 0x80 unchanged, and a 1,000,000-iteration spin window with all enables clear records 0 traps. 17 checks, 0 mismatches, FNV-1a 0x283c26a67ca35455 byte-identical across 3 QEMU 8.2.2 runs. [src/mip-msip-write/](src/mip-msip-write/)
- **src/fflags-nv-invalid/**: clears fflags, executes an invalid floating-point operation, and verifies the NV bit sets while no other accrued flag moves. [src/fflags-nv-invalid/](src/fflags-nv-invalid/)
...
- **src/mstatus-tsr-trap/**: proves mstatus.TSR (bit 22) traps an S-mode sret into M-mode as illegal-instruction (mcause 0x2 at the sret site 0x800002e4), while the same sret returns to S-mode with TSR clear (phase 1: exactly 1 trap, the S-mode ecall with mcause 0x9). Setting TSR via csrs reads back set with no other mstatus bit disturbed, and boot mstatus 0xa00000000 restores bit-for-bit. 14 checks, 0 mismatches, checksum 0x48849bcd4202266a, byte-identical across 3 QEMU 8.2.2 runs. [src/mstatus-tsr-trap/](src/mstatus-tsr-trap/)
...
- **src/pmp-na4-match/**: proves a locked NA4 PMP entry matches exactly its 4-byte word: an lbu inside the word traps as a load access fault (mcause 0x5, mtval at the word), while an lbu one byte past it matches no entry and completes under M-mode default-allow; the faulting lbu traps with mcause 0x5, mepc at the faulting instruction and mtval 0x80002000, the faulting destination keeps its poison value, the neighbor word reads back 0x9f8e7d6c and the canary+4 byte returns 0x6c; the lock makes the programmed values (pmpaddr0 0x20000800, pmpcfg0 0x90) persist bit-for-bit through restore attempts. 21 checks, 0 mismatches, FNV-1a 0xb3bbd7eb5f7a5e74 identical on 3 QEMU 8.2.2 runs, Verdict PASS. [src/pmp-na4-match/](src/pmp-na4-match/)
- **src/pmp-na4-low-edge/**: proves a locked NA4 PMP entry matches exactly its 4-byte word on the low edge: an lbu one byte below the word matches no entry and completes with no trap under M-mode default-allow, while an lbu at the word's first byte traps as a load access fault (mcause 0x5, mtval at the word). Measured: entry 0 programmed as locked NA4 deny (pmpaddr0 0x20000900, pmpcfg0 0x90) over the word at 0x80002400; lbu at 0x800023ff returned 0x12 with 0 traps, lbu at 0x80002400 trapped with mcause 0x5, mepc 0x80000614, mtval 0x80002400, destination kept poison 0xdeadbeefdeadbeef, neighbor word 0x12345678 unchanged; quiet window 10000 reads, 0 new traps; lock-persistence readbacks bit-for-bit. 21 checks, 0 mismatches, FNV-1a 0xc31c8a92d948b7ba, 3 QEMU 8.2.2 runs byte-identical, Verdict PASS. [src/pmp-na4-low-edge/](src/pmp-na4-low-edge/)
